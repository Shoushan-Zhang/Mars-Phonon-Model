...
!
      ...
      integer :: iwrite_rayparam
      integer :: iwrite_scatevent
      ...
      print *,'The radius is set to : ',erad
      print *,'The maximum pcore slowness is set to: ',pcoremax,'s/deg'
      print *,'Maximum layer number allowed: ',nlay0, '; Maximum ray number: ',nray0
      print *,'Maximum number of interfaces,scattering volumes, and Q layers allowed: ', nface0
      print *,'ntdim,nxdim = ',ntdim,nxdim
      print *,'-------------------------------------------------------------------------'
...
! Thus overlapping depth ranges can be used
	 ...
	 print *,'Velocity ratio = ', gam0(i)
         ...
         read *,eps(i)
         if (eps(i).gt.rms_max) rms_max=eps(i)
      ...
      print *,'If writing ray parameter output? (0) no, (1) yes'
      read *,iwrite_rayparam

      print *,'If writing scattering event output? (0) no, (1) yes'
      read  *,iwrite_scatevent
...
! avoid singularity in transformation
      ...
      open (12,file='flattened_vel.txt',status='replace')
      write(12,'(A)') '  i     z_s    alpha_s     z_flat    alpha     beta_s     beta'
      do 10 i=1,nlay0
         ...
         write(12,'(I4,2X,F6.2,2X,F8.2,2X,F8.2,2X,F8.2,2X,F8.2,2X,F8.2)') &
              i, z_s(i), alpha_s(i), z(i), alpha(i), beta_s(i), beta(i)
...
!     &      alen(iscat), gpp0,gps0,gsp0,gss0)
          ...
          call GAVGSATO2(el(iscat),nu(iscat),gam0(iscat),eps(iscat), &
           ...
           gppm,gpsm,gspm,gssm)
            ...
            print *,'  afp=1./(gpp0+gps0), 1./(gsp0+gss0) ', 1./(gpp0+gps0), 1./(gsp0+gss0)
            print *,'  gppm,gpsm,gspm,gssm= ',gppm,gpsm,gspm,gssm
            print *,'  mom. afp=1./(gppm+gpsm), 1./(gspm+gssm) ', 1./(gppm+gpsm), 1./(gspm+gssm)
...
!---------------------------------------------------------------------
         ...
         if (iwrite_rayparam.eq.1) then
            open(unit=10, file='ray_output_P.txt', status='unknown')
            open(unit=11, file='ray_output_S.txt', status='unknown')
            write(10,'(A)') 'RayIndex  Layer  TravelTime(s)  TravelDist(km)  InitAmp'
            write(11,'(A)') 'RayIndex  Layer  TravelTime(s)  TravelDist(km)  InitAmp'
         end if
      ...
      if (iwave.eq.1) then
         vel0=vpsource
         !pmax=1./(vpmin+0.0001)
         ! Take rms velocity fluctation into slowness calculation
         pmax = 1./(vpmin * (1-rms_max))  
      else
         vel0=vssource
         pmax=1./(vsmin+0.0001)
         !pmax = 1./(vsmin)
         pmax = 1./(vsmin * (1-rms_max))
      end if
...
!      stop
      ...
      do 100 i=1,npts-1
         ...
         do 50 ip=1,np
            ...
            if (iwrite_rayparam.eq.1) then
               if (iw.eq.1) then
                  write(10,'(I8,1X,I6,1X,F12.6,1X,F12.6,1X,F12.6)') ip, ilay, dt(ilay,ip,iw), dx(ilay,ip,iw), amp0(ip,iw)
               else
                  write(11,'(I8,1X,I6,1X,F12.6,1X,F12.6,1X,F12.6)') ip, ilay, dt(ilay,ip,iw), dx(ilay,ip,iw), amp0(ip,iw)
               endif
            endif
...
110   continue    !end loop on iwave for P and S
      ...
      nray=0
      nsurf=0
 print *,'Maximum time allowed: tmax= ',tmax,' seconds'
!--------------------------------------------------------------------
!--------------------------------------- start spraying rays
...
!--------------------SCATTERING SECTION-------------------------------------
            if (distsum.ge.freepath) then           !SCATTERED
               ...
               if (iwrite_scatevent.eq.1) then
                  open(unit=19, file='scatevent.txt', status='unknown', action='write', form='formatted', position='append')
                  write(19,'(A, I2, 2X, A, I6, 2X, A, I6, 2X, A, I4, 2X, A, I4, 2X, A, F10.4, 2X, A, F10.4, 2X, A, F10.4,&
                   2X, A, F10.4, 2X, A, F10.4, 2X, A, F10.4)') &
                  'Wave Type:', iw, 'Ray Index:', ip, 'Scattering Event #:', nscat, 'Layer:', iscat, 'Depth Index:', ilay, &
                  'Depth(km):', z_s(ilay), 'DistSum(km):', distsum, 'FreePath(km):', freepath, &
                  'Travel Time (s):', dt(ilay,ip,iw), 'Total Time (s):', t, 'Total Distance (km):', x
                  close(19)
               end if
...
210            if (idir.eq.1) then    !scat point at end of ray path
               ...
               if (vs0.eq.0) then
                  print *,''
                  print *,'******** Problem with vs0 = 0 at scatterer *********'
                  print *,'ilay=',ilay
                  print *,'idir,ip,iwave1,iwave2 = ',idir,ip,iwave1,iwave2
                  print *,'vp0,vs0 = ',vp0,vs0
                  print *,'beta_s(ilay)=',beta_s(ilay)
                  print *,'beta(ilay)=',beta(ilay)
                  if (idir.eq.1) then
                     print *,'vs0 = beta(ilay+1)'
                  else
                     print *,'vs0 = beta(ilay)'
                  endif

                  print *,'***Set vs0 to vsmin***'
                  print *,'vsmin',vsmin
               endif
...
350   continue                    !not end of do loop
      ...
      if (mod(nray,nraydump).eq.0) then
         print *,'writing rbin files...   nray,nraydump,nsurf(number of particles arriving surface) = ',nray,nraydump,nsurf
...
490         continue
500      continue
         print *,'n(non zero values in rbin),rmax,rsum(summation of nonzero values) = ',n,rmax,rsum
...
10    continue
      ...
      if (ip.le.0.or.ip.gt.np+20) then
         ...
         open(unit=99,file='SCATRAYPOL problema.txt',status='replace',position='append',action='write')
         write(99,*) '==== SCATRAYPOL problema ===='
         write(99,*) 'This log entry captures values when ray index ip is out of bounds(<=0 or >=np+20).'
         write(99,*) 'ip (scattered ray index)         =', ip
         write(99,*) 'np (total rays in table)         =', np
         write(99,*) 'frac (interpolation fraction)    =', frac
         write(99,*) 'iw1 (incident wave type)         =', iw1
         write(99,*) 'iw2 (scattered wave type)        =', iw2
         write(99,*) 'ptarg (target p2)                =', ptarg
         write(99,*) 'p(1,iw2) (min p)                 =', p(1,iw2)
         write(99,*) 'p(np,iw2) (max p)                =', p(np,iw2)
         write(99,*) 'the (takeoff angle, deg)         =', the
         write(99,*) 'slow(iw2) (vertical slowness)    =', slow(iw2)
         write(99,*) 'b3 (ray direction vector)        =', b3(1), b3(2), b3(3)
         write(99,*) 'a3 (incident ray dir vector)     =', a3(1), a3(2), a3(3)
         write(99,*) 'a1 (SV pol. direction)           =', a1(1), a1(2), a1(3)
         write(99,*) 'svsh0 (original S polarization)  =', svsh0
         write(99,*) 'spol (scattered polarization)    =', spol
         write(99,*) 'psi (scattering angle from ray)  =', psi
         write(99,*) 'zeta (scattering angle from x1)  =', zeta
         write(99,*) 'vp0 (P velocity at scatter point)=', vp0
         write(99,*) 'vs0 (S velocity at scatter point)=', vs0
         write(99,*) 'idir (incident ray direction)    =', idir, ' (1 = downgoing, 2 = upgoing)'
         write(99,*) 'azi0 (incident azimuth, deg)     =', azi0
         close(99)  
...
!
      ...
      real, parameter :: pi=3.141592654
      real, parameter :: e =2.718281828
          expsato=(2.*pi**2 * eps**2 * a**3 ) / ((1.+a**2*m**2)**1.5)
      return
      end
