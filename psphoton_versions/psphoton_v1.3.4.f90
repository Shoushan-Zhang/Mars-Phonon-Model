...
! Thus overlapping depth ranges can be used
         ...
         read *,alen(i)
         print *,'Enter Hurst coefficient (e.g., 0.5)'
         read *,HC(i)
      enddo
      ...
      do i=1,nqlay
         ...
         read *,qalpha(i)
         print *,'Enter Qs2Qp ratio for layer: ',i
         read *,Qs2Qp(i)
      enddo
      ...
      write(12,'(A)') '   i    z_s    alpha_s    z_flat     alpha     beta_s     beta       Qp       Qs'
      ...
      do 10 i=1,nlay0
         ...
         call FLATTEN(z_s(i),alpha_s(i),z(i),alpha(i),erad)
         if (beta_s(i).ne.0.) then
            call FLATTEN(z_s(i),beta_s(i),z(i),beta(i),erad)
         else
            beta(i)=0.
         end if
         iflag(i)=0
         ...
         do k=1,nqlay
            if (z_s(i).ge.zminq(k).and.z_s(i).le.zmaxq(k)) then
               q(i,1)=qalpha(k)
               q(i,2)=Qs2Qp(k)*q(i,1)  !set Q for S using Qs2Qp ratio
            end if
         enddo
         
         ! q(i,1) is Q for P, q(i,2) is Q for S
         ...
         write(12,'(I4,2X,F6.2,2X,F8.2,2X,F8.2,2X,F8.2,2X,F8.2,2X,F8.2,2X,F8.1,2X,F8.1)') &
     i, z_s(i), alpha_s(i), z(i), alpha(i), beta_s(i), beta(i), q(i,1), q(i,2)
...
!     &      alen(iscat), gpp0,gps0,gsp0,gss0)
          ...
          call GAVGSATO2(el(iscat),nu(iscat),gam0(iscat),eps(iscat), &
           alen(iscat), HC(iscat), gpp0,gps0,gsp0,gss0, &
           gppm,gpsm,gspm,gssm)
...
110   continue    !end loop on iwave for P and S
      ...
      nray=0         !how many rays sprayed
      nsurf=0        !how many rays hit the surface
!
!--------------------------------------------------------------------
!--------------------------------------- start spraying rays
...
150   do 800 iw_init=iww1,iww2
         do 780 idir_init=idirbeg,idirend,2 
            ...
            do 760 ipp=1,np
               ip=ipp
               iw=iw_init
               if (iwstart.eq.7.and.p(ip,iw).gt.pmax_takeoff) go to 760     !The takeoff angle is too large
               idir=idir_init
            ...
            if (distsum.ge.freepath) then           !SCATTERED
               ...
               if (fran.le.scatprob(iscat,iw,1)) then
                  call RANDANG2(iscat,iwoff+1,el(iscat),nu(iscat), &
                    gam0(iscat),eps(iscat),alen(iscat),HC(iscat),psi2,zeta2,spol)
                  iwave2=1
               else
...
! FLATTEN calculates flat earth tranformation.
      subroutine FLATTEN(z_s,vel_s,z_f,vel_f,erad)
      implicit none
...
!
      subroutine RANDANG2(iscat,itype,el,nu,gam0,eps,a,HC,psi,zeta,spol)
      implicit none
      ...
      if (firstcall(iscat)) then
         ...
         do 20 i=0,n
            do 10 j=-n,n
               ...
               call GSATO(psi2(k,iscat),zeta2(k,iscat),el,nu,gam0,eps,a, HC ,&
...
!
       subroutine GAVGSATO(el,nu,gam0,eps,a,HC, &
             gpp0,gps0,gsp0,gss0)
      implicit none
      ...
      do 100 i=0,n
         ...
         do 90 j=-n,n
            ...
            call GSATO(psi,zeta,el,nu,gam0,eps,a, HC ,&
...
!            eps  =  RMS velocity perturbation
!            a    =  correlation distance
!            HC   =  Hurst coefficient
!   Returns: gpp0,gps0,gsp0,gss0  =  averaged over solid angle
!            gppm,gpsm,gspm,gssm  =  momentum scattering coef.
!
       subroutine GAVGSATO2(el,nu,gam0,eps,a,HC, &
             gpp0,gps0,gsp0,gss0, &
             gppm,gpsm,gspm,gssm)
      implicit none
      real :: psi,zeta,el,nu,gam0,eps,a,gpp,gps,gsp,gss,spol
...
!                 =  0 for pure psi direction
!
       subroutine GSATO(psi,zeta,el,nu,gam0,eps,a,HC, &
             gpp,gps,gsp,gss,spol)
      implicit none
      real :: psi,zeta,el,nu,gam0,eps,a,gpp,gps,gsp,gss,spol
      ...
      gpp = (el4/pi4)*xpp**2*EXPSATO(eps,a,arg,HC)
      if (gpp.lt.1.e-30) gpp=0.

      arg=(el/gam0)*sqrt(1.+gam2-2.*gam0*cos(psi))
      gps = (1./gam0)*(el4/pi4) * xps**2 * EXPSATO(eps,a,arg,HC)
      if (gps.lt.1.e-30) gps=0.

      gsp = gam0*(el4/pi4) * xsp**2 * EXPSATO(eps,a,arg,HC)
      if (gsp.lt.1.e-30) gsp=0.

      arg=2.*el*sin(psi/2.)
      gss = (el4/pi4)*(xss_psi**2+xss_zeta**2)*EXPSATO(eps,a,arg,HC)
...
!
      real function EXPSATO(eps,a,m,HC)
      implicit none
      real :: eps,a,m,HC
